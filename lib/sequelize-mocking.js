...
class SequelizeMocking {
    ...
    static async loadFixtureFile(sequelize, fixtureFilePath, options) {
        ...
        if(Array.isArray(fixtureFilePath) ){

            for(let i = 0; i < fixtureFilePath.length; i++){

                await sequelizeFixtures
                    .loadFile(fixtureFilePath[i], SequelizeMocking.mapModels(sequelize), loadFixturesOptions);
            ...
            await sequelizeFixtures
                .loadFile(fixtureFilePath, SequelizeMocking.mapModels(sequelize), loadFixturesOptions);
        }
